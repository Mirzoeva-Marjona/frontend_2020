...
function task2(employees) {
	...
	for (let i = 0; i < employees.length; i++) {
		...
		if (map.has(position)) {
			let num = map.get(position);
			map.set(position, ++num);
		} else {
			map.set(position, 1);
		}
	}
